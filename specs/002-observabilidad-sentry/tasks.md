...
- [X] T045 [US7] (manual) En Sentry → `aeropass-back` → Settings → Security & Privacy: activar "Prevent Storing of IP Addresses" y confirmar que el Data Scrubbing por defecto está activo. Registrar en `specs/002-observabilidad-sentry/contracts/dashboard-and-alerts.md` §Estado. *Hecho: IP apagadas por API; Data Scrubbing ya activo.*
- [x] T046 [US7] (manual) Configurar en Vercel `SENTRY_DSN`, `SENTRY_ENVIRONMENT` (`prod` en Production, `dev` en Preview) y `SENTRY_TRACES_SAMPLE_RATE` (`0.2` en Production, `1.0` en Preview), redesplegar y verificar quickstart §3 y §4 (evento en menos de 1 minuto; `/health` 200). Si el evento solo llega con el siguiente request, revisar T015 (`wait_until`). *Pendiente: requiere acceso a Vercel (colega del equipo). Ver el documento de traspaso.*
- [X] T047 [US7] Correr `seed_demo` con `SENTRY_ENVIRONMENT=simulated` (y `--contingencia`) para tener datos con los que crear widgets y monitores. *Hecho con un Postgres local (base `aeropass_demo`) y el DSN solo como variable de entorno.*
- [X] T048 [US7] (manual) Crear el monitor de Uptime sobre `https://<prod>/health` (GET, cada 1 min, timeout 10 s, 3 fallos para abrir, 1 éxito para cerrar, entorno `prod`, correo al equipo `#aeropass-team`) según contracts/dashboard-and-alerts.md. Registrar el ID. *Hecho (2026-09-24): monitor `10437854`; alerta B4 `6063447` conectada al monitor (correo a `#aeropass-team`); widget W5 en el dashboard `10191181` como texto con el enlace al monitor, porque el editor de widgets no ofrece dataset de Uptime (alternativa del contrato).*
- [X] T049 [US7] (manual) Crear el dashboard "AeroPass Backend — Observabilidad" con los widgets W1–W11 y el filtro de entorno según el contrato. Si W3 no admite ecuación o W5 no tiene dataset de Uptime, aplicar la alternativa escrita en el contrato y anotarla. Registrar la URL. *Hecho: dashboard `10191181`, W1–W11 (W5 agregado en T048).*
- [X] T050 [US7] (manual) Crear las alertas B1 (issues con `error.unhandled:true`, todos los entornos, 1 correo cada 5 min), B2 (autoservicio < 0,85 en 1 h, solo `prod`; si no admite la ecuación, aplicar la alternativa del contrato), B3 (≥1 apertura de circuit breaker en 10 min, todos los entornos) y B4 (la del monitor de Uptime). Todas con correo al equipo `#aeropass-team`. Registrar los IDs. *Hecho: B1 `6062331`, B2 monitor `10436324` + alerta `6062356` (entorno `prod` fijado por API), B3 monitor `10436299` + alerta `6062338`. B4: alerta `6063447` (en T048).*
- [X] T051 [US7] Verificar con `seed_demo --solo-error` en `simulated` que llega el correo de B1 en menos de 2 minutos, y que W3, W4, W7 y W8 coinciden con las cifras impresas por la herramienta. Actualizar §Estado del contrato con los resultados. *Hecho: B1 se disparó 26 s después del error (issue `AEROPASS-BACK-2`, mensaje redactado). Las cifras coinciden con las de la herramienta (87,5 % y 36,4 %). La llegada del correo la confirma el equipo.*
...
- [X] T055 Medir el impacto del arranque en frío (SC-004): tiempo de importación de `aeropass.main` con y sin `SENTRY_DSN` (`python -X importtime`) y latencia de `GET /health` en el deploy. Anotar los resultados en `specs/002-observabilidad-sentry/research.md` §3. *Parte local hecha: la inicialización de Sentry bajó de 436 ms a 45 ms con `auto_enabling_integrations=False` (research §3). Latencia de `/health` en el deploy medida el 2026-09-24: 4 requests consecutivos, 1.35–1.49 s cada uno (ver research §3); no se distingue arranque en frío en esta ráfaga porque la función ya estaba caliente por un request previo.*
- [ ] T056 Recorrer `specs/002-observabilidad-sentry/quickstart.md` §1–§7 y marcar lo verificado. §3, §4 y §7 requieren un deploy (T046). *Parcial (2026-09-24), ver quickstart §Estado de verificación: §1, §2 y §4 verificados; §5 y §6 verificados en `simulated`. Falta §3 completo (`e2e_flow` contra un deploy con `BIOMETRIC_PROVIDER=mock`; por ahora solo se sabe que el SDK de `prod` envía transacciones) y §7 (redeploy + logs de Vercel).*
