...
- [ ] T048 [US7] (manual) Crear el monitor de Uptime sobre `https://<prod>/health` (GET, cada 1 min, timeout 10 s, 3 fallos para abrir, 1 éxito para cerrar, entorno `prod`, correo al equipo `#aeropass-team`) según contracts/dashboard-and-alerts.md. Registrar el ID. *Parcial (2026-09-24): monitor `10437854` creado. Faltan la alerta B4 (conectar el monitor a una regla de email a `#aeropass-team`, bloqueado por permisos del conector) y el widget W5 en el dashboard `10191181` (sin herramienta de escritura de widgets en el conector). Ambos quedan manuales en la UI de Sentry.*
- [X] T049 [US7] (manual) Crear el dashboard "AeroPass Backend — Observabilidad" con los widgets W1–W11 y el filtro de entorno según el contrato. Si W3 no admite ecuación o W5 no tiene dataset de Uptime, aplicar la alternativa escrita en el contrato y anotarla. Registrar la URL. *Hecho: dashboard `10191181`, W1–W4 y W6–W11. W5 queda en T048.*
- [X] T050 [US7] (manual) Crear las alertas B1 (issues con `error.unhandled:true`, todos los entornos, 1 correo cada 5 min), B2 (autoservicio < 0,85 en 1 h, solo `prod`; si no admite la ecuación, aplicar la alternativa del contrato), B3 (≥1 apertura de circuit breaker en 10 min, todos los entornos) y B4 (la del monitor de Uptime). Todas con correo al equipo `#aeropass-team`. Registrar los IDs. *Hecho: B1 `6062331`, B2 monitor `10436324` + alerta `6062356` (entorno `prod` fijado por API), B3 monitor `10436299` + alerta `6062338`. B4 queda en T048.*
